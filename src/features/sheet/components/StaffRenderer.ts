...
export class StaffRenderer {
  ...
  private drawTitle(tempo: number) {
    ...
    this.ctx.font = "600 24px Inter, system-ui, -apple-system, sans-serif";
    this.ctx.fillText("Interactive Composition", 30, 50);

    this.ctx.font = "400 14px Inter, system-ui, -apple-system, sans-serif";
  ...
  private drawStaffLines() {
    for (let measure = 0; measure < staffConfig.measures; measure++) {
      ...
      if (measure === staffConfig.measures - 1) {
        ...
        this.ctx.beginPath();
        this.ctx.moveTo(
          measureX + staffConfig.measureWidth,
          staffConfig.topMargin,
        );
        this.ctx.lineTo(
          measureX + staffConfig.measureWidth,
          staffConfig.topMargin + staffConfig.staffHeight,
        );
        this.ctx.stroke();
      }
    }
  ...
  private drawClefAndTimeSignature() {
    ...
    this.ctx.font = "600 30px Inter, system-ui, -apple-system, sans-serif";
  ...
  private drawNotes(
    ...
    notes.forEach((note, index) => {
      ...
      const isStart = index === playbackStartIndex && currentNoteIndex === -1;
      this.drawNote(note, isCurrent, isStart);
    });
  }

  private drawNote(note: SheetNote, isPlaying: boolean, isStartPoint: boolean) {
  ...
  private drawLedgerLines(note: SheetNote, x: number) {
    ...
    if (note.staffPosition < 0) {
      for (let pos = -0.5; pos >= note.staffPosition; pos -= 1) {
        const ledgerY =
          staffConfig.topMargin + pos * staffConfig.lineSpacing + staffConfig.staffHeight / 2;
        this.ctx.beginPath();
  ...
  private drawNoteHead(note: SheetNote, x: number, y: number, isPlaying: boolean) {
    const noteColor = isPlaying ? "#6366f1" : "#020817";

    this.ctx.fillStyle =
      note.duration === "whole" || note.duration === "half" ? "#ffffff" : noteColor;
  ...
  private drawStem(note: SheetNote, x: number, y: number, isPlaying: boolean) {
