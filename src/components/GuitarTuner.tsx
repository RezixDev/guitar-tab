...
export function GuitarTuner() {
  ...
  const [centsOffPitch, setCentsOffPitch] = useState(0);
  ...
  const toggleMicrophone = () =>
    isMicActive ? stopMicrophoneInput() : startMicrophoneInput();
                    ...
                    <div className="space-y-2 mt-4">
                      <Progress
                        ...
                        aria-valuetext={
                          tuningStatus === "in-tune"
                            ? "In Tune"
                            : `${Math.abs(Math.round(centsOffPitch))} cents ${tuningStatus}`
                        }
                      />
              ...
              {STANDARD_TUNING.map((note) => (
                <div key={note.string} className="flex flex-col items-center">
                  <Button
                    ...
                    onClick={() =>
                      isPlaying && selectedNote?.note === note.note
                        ? stopNote()
                        : playNote(note)
                    }
